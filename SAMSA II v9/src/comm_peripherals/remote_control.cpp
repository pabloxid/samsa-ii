...
void RemoteControl::conductas (byte comando) {
	...
	switch (comando) {
		...
		case 0: case 1: case 2: case 3:
			if (!isMoving) {
				texto1 = "cond. " + String (comando, DEC);
				if (conducta[comando]->enabled) {
					conducta[comando]->disable ();
					texto1 += " OFF";
				} else {
					conducta[comando]->enable ();
					texto1 += " ON";
				} 
			} 
		  break;
