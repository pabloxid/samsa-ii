...
// funciones
void set_coord (MOVDATA &m);                    // ordena una posición
byte motor_update ();                          // actualiza la posición de los motores
void motor_setup ();                          // inicializa los motores
COORD3D get_coord (byte pata);                  // devuelve las coordenadas actuales de una pata
void init_filters ();                         // inicializa los filtros para el load
void poll_load ();                            // lee continuamente el load de los motores
void poll_analog_sensor (unsigned long milis);      // lee continuamente los sensores analógicos (cuerdita)
