...
void Movimiento::caminata (float velocidad, float desplazamiento, bool curva, COORD2D centro, float angulo, byte marcha, float largo_pasos) {
  ...
  float rotacion;
  if (!curva) {
    vector = (COORD2D) {modulo_vector*cos(angulo), modulo_vector*sin(angulo)};
    rotacion = 0;
  } else {
    vector = centro;
...
// idea: tanto para la "modulación" como para arrancar caminatas desde cualquier posición y que lentamente se vaya acomodando
// la clave es: transformar pos_ref_

void Movimiento::update (unsigned long milis) {
	
	// variables de control generales
		static unsigned long timer = 0;                            // timer
	// variables geométricas internas de la caminata
		static float ciclo, coeficiente;
   static COORD2D rotor1 [2], rotor2 [2];                      // talvez convendría liberar estos arrays mientras no se usan
	// variables de control para la caminata
   static byte index;                                       // índice de las fases 
   static byte apoyadas;                                     // patas apoyadas 
   static byte counter [MAX_FASES];                            // cuando vale 0, la pata se considera apoyada
   static COORD3D comp [6], pos_ref[6];                        // para la compensación de posición de referencia
	// variables para el tronco
		static float param_tronco [] = {0,0,0,0,0,0,0,0,0};            // matriz de 9 parámetros para el movimiento del tronco
  ...
  if (init) {                                                  // init es el flag que indica que se deben actualizar las variables de control 
       ...
       if (rotacion_ == 0) {
			    vector_ = producto (vector_, -1);                 // lo invierte para acelerar los cálculos
			  } else {
         getRotationMatrix (rotor1, -rotacion_);                // matriz de rotación del cuerpo
         getRotationMatrix (rotor2, rotacion_*coeficiente);       // matriz de rotación de los "pasitos". Para hacer una "zancada progresiva" habría que recalcular esta matriz todo el tiempo
       }
       // variables de control de la caminata
			  for (byte f=0; f<fases_; f++) {counter [f] = duracion_pasos_;}    // empieza valiendo duracion_pasos
				...
				if (compensate_) {
				  for (byte pata=0; pata<6; pata++) {
						comp [pata] = producto(resta(pos_ref_[pata], pos_des[pata]), 1.0/ciclo);
						memcpy (pos_ref, pos_des, 6*sizeof(COORD3D));
					} 
				} else {
  ...
  // empieza el loop (bloque activo)
	if (tick < ticks_ && enable) {
		switch (mode) {
			case CAMINATA: {
    
				// compensación de posición de referencia
				if (compensate_ && tick < ciclo) {
				  for (byte pata=0; pata<6; pata++) {
				...
				for (byte i=0; i<fases_; i++) {
					if (counter[i] > 0) {counter[i]--;} else {apoyadas += secuencia_[i];}
				}
...
// aumneta la velocidad en movimiento
