...
byte minimo (RingBuffer* buffer, byte from, byte to) {                             // devuelve la posición del mínimo en el rango
  byte min = from;
  for (byte i = from + 1; i < to; i++){                                                                   
...
float blend (float val1, float val2, float blend) {
...
void sample () {                                    // adquiere la información 
  ...
  switch (ciclo) {
    ...
    case 0: {
      ...
      else if (palmer > 9 && palmer <= 11) {distancia = blend (palmer, sharp, (11-palmer)/2);} 
