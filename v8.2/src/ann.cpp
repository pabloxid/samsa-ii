...
float Nnode::compute (float* input) {
	...
	for (byte n=0; n<n_inputs; n++) {
		accumulator += input[n]*weight[n];
	}
	accumulator += weight[n_inputs];                             // procesa el 'bias weight'
	// if (accumulator < -22.52) {accumulator = -22.52;}                  // esto es por algún bug 
	return sigmoide (accumulator);                               // sigmoid
}
